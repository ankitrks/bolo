...
def cache_follow_post(user_id):
    ...
    topic_ids = list(Topic.objects.filter(user_id__in=all_follower, is_vb = True, is_removed = False, is_popular = False)\
    ...
    query = Topic.objects.filter(id__in=topic_ids).order_by('-id', '-vb_score')
