...
def rename_s3_file(old_key,new_key):
    ...
    s3.Object(settings.BOLOINDYA_AWS_BUCKET_NAME, new_key).copy_from(CopySource=old_key,ACL='public-read')
    s3.Object(settings.BOLOINDYA_AWS_BUCKET_NAME, old_key).delete()

def run():
    for each_topic in Topic.objects.filter(is_vb=True).order_by('-id'):
