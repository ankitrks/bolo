...
def put_share_data(start_date):
    ...
    try:
        all_data = UserVideoTypeDetails.objects.filter(timestamp__date = start_date)
...
def put_videos_created(today, start_date):
    ...
    day_month_year_dict = dict()
    all_data = Topic.objects.filter(date__date = start_date)
    for item in all_data:
...
def put_video_creators_analytics_lang(today, dt):
        ...
        # for dt in rrule.rrule(rrule.DAILY, dtstart = start_date, until = today):
        try:
            all_data = Topic.objects.filter(is_vb=True, date__date = dt).values('user', 'language_id', 'm2mcategory')\
...
def put_total_video_creators(today, dt):
        ...
        str_date = str(dt.year) + "-" + str(dt.month) + "-" + str(dt.day)
        all_data = Topic.objects.filter(is_vb = True, date__date = dt).values('user', 'pk', 'language_id', 'm2mcategory').order_by('user') 
        for item in all_data:
...
def fixed_call():
    # put_mau_data(datetime.today().date())
    days_to_run = 79
