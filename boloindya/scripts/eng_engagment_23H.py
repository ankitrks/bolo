...
def run():
    print "Start Time Eng_Engagment 23H: ",datetime.now()
    ...
    for each_seen_id in last_n_days_post_ids:
        try:
            ...
            if each_seen.date +timedelta(hours=21) < now and each_seen.date +timedelta(hours=24) > now and each_seen.view_count < 30000:
            ...
            pass
    print "End Time Eng_Engagment 23H: ",datetime.now()
...
def check_like(topic_id,user_ids):
    ...
    if each_like.likes_count < each_like.view_count/random.randrange(4,25):
        if each_like.date +timedelta(hours=21) < now and each_like.date +timedelta(hours=24) > now and each_like.view_count/random.randrange(4,25) > 1000:
