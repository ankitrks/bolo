...
def run():
    ...
    BoloActionHistory.objects.filter(action_object_type=vb_like_type).exclude(action_object_type=vb_like_type,action_object_id__in=all_vb_social_share_id).delete()
    print "after: BoloActionHistory delete object no exist",datetime.now()
