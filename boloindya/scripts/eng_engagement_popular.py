...
def run():
    ...
    now = datetime.now()
    last_n_days_post_ids = Topic.objects.filter( Q(popular_boosted_time__gte = now-timedelta(days=5))|Q(date__gte =now-timedelta(days=5) ), is_popular = True, is_vb=True, is_removed=False, user__st__boosted_time__isnull=True).order_by('-date').values_list('id',flat=True)
