...
class CategoryWithVideoSerializer(ModelSerializer):
    ...
    def get_topics(self,instance):
        topics = []
        all_seen_vb = []
        is_expand = self.context.get('is_expand', True)
        language_id = self.context.get('language_id', 1)
        user_id  = self.context.get('user_id', None)
        page =  int(self.context.get('page', 0))
        filter_dict = {'m2mcategory' : instance, 'language_id' : language_id}
        excluded_dict = {'vb_score__gte' : self.context.get('vb_score', None)}
        topics = get_ranked_topics(user_id, page, filter_dict, excluded_dict)
        return CategoryVideoByteSerializer(topics[:settings.REST_FRAMEWORK['PAGE_SIZE']], many = True, context = {'is_expand' : is_expand}).data
