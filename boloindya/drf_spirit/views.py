...
def get_category_with_video_bytes(request):
    try:
        ...
        if request.GET.get('is_with_popular'):
            startdate = datetime.today()
            enddate = startdate - timedelta(days=15)
            topics = Topic.objects.filter(is_removed=False, is_vb=True, is_popular=True, language_id=language_id, date__gte=enddate).order_by('-view_count', '-comment_count')
...
def get_popular_video_bytes(request):
    try:
        ...
        try:
            ...
            all_seen_vb = []
        topics = []
        topics_not_seen = Topic.objects.filter(is_removed=False, is_vb=True, is_popular=True, language_id=language_id, date__gte=enddate).exclude(id__in=all_seen_vb).order_by('-view_count', '-comment_count')
        topics_seen = Topic.objects.filter(is_removed=False, is_vb=True, is_popular=True, language_id=language_id, date__gte=enddate, id__in=all_seen_vb).order_by('-view_count', '-comment_count')
