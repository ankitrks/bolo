...
def get_hash_discover(request):
    try:
        ...
        hashtag_languages = ['0']+[language_id]
        hash_tags = HashtagViewCounter.objects.exclude(hashtag__is_blocked = True).filter(language=language_id, hashtag__languages__overlap=hashtag_languages).order_by('-hashtag__is_popular', 'hashtag__order',\
...
class AudioFileListView(generics.ListAPIView):
    ...
    queryset = MusicAlbum.objects.filter(is_extracted_audio=False)
    permission_classes  = (IsAuthenticatedOrReadOnly,)
    pagination_class = LimitOffsetPagination
